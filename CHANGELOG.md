...
- Потеря таймзоны при сохранении `received_at`. Заголовок `Date:` несёт офсет
  отправителя (например, `+03:00`), а datetime-каст Eloquent пишет wall-clock как
  есть и читает обратно как app-таймзону — момент письма смещался ровно на
  величину офсета (письмо в `10:34 +03:00` сохранялось как `10:34` UTC вместо
  `07:34`), что давало характерное расхождение `received_at` и `created_at` у
  одной записи. `StoreIncomingMessage` теперь приводит момент к
  `config('app.timezone')` перед записью, зеркаля `now()`/`created_at`. Путь
  архива (`{Y/m/d}`) строится из того же нормализованного момента, поэтому
  остаётся консистентным с колонкой.

## [3.9.1] - 2026-06-18

### Changed

- Логирование отфильтрованного письма вынесено из захвата (`StoreIncomingMessage`)
  в отдельный дефолтный слушатель `Listeners\LogFilteredMessage`, подписанный на
  событие `MessageFiltered`. Захват больше не принимает решения о политике
  логирования — он только объявляет событие. Поведение по умолчанию не меняется
  (лог `info` на каждое отфильтрованное письмо), но новый флаг
  `MAILSPOON_LOG_FILTERED=false` (`mailspoon.log_filtered`) отключает дефолтный
  лог, оставляя логирование на усмотрение подписчика события.
...
- `mailspoon:doctor`: проверка `schedule` теперь оценивает расписание и у
  паузнутого ящика (`enabled => false`), а не проходит молча. Doctor — preflight,
  его часто гоняют во время настройки, пока ящик ещё выключен, поэтому пауза без
  cron-расписания тоже даёт предупреждение, а не прячет недонастройку. Сообщение
  для паузнутого ящика теперь точное: он не опрашивается ни в одном режиме (cron,
  ручной `pull` и `sentry` все пропускают выключенный маршрут).
