...
> ⬜ Предложение. Разблокировано переходом на пакет (#21): mailspoon — библиотека
> внутри хост-приложения, и логично дать хосту регистрировать маршруты в
> рантайме, не редактируя опубликованный `config/mailspoon.php`.
...
- [ ] README: раздел «Регистрация маршрутов в рантайме» — `Mailspoon::register()`
      и `Imap::register()` из `boot()`, пример из БД, стоимость `boot()` и
      кэширование, encrypted-cast для паролей, семантика времени.
...
- [ ] Без `register()` mailspoon работает как сейчас (тесты зелёные, паритет).
- [ ] `Mailspoon::register()` добавляет/переопределяет маршрут (endpoint, key,
      enabled, mark, filters, schedule) в рантайме.
- [ ] Хост может опрашивать ящик не из `config/imap.php`, зарегистрировав
      аккаунт через `Imap::register()` в `boot()` (без правок imapengine).
- [ ] Тесты на регистрацию маршрута (захват, доставка, планировщик).
- [ ] README: пример из БД, стоимость `boot()`, безопасность паролей, семантика.