...
> Актуализация (2.0/3.0): исходы разнесены по фазам store-and-forward —
> `filtered` известен при захвате (соединение с ящиком открыто, действие можно
> выполнить сразу), а `delivered`/`failed` наступают позже в
> `mailspoon:deliver`, когда IMAP-соединения нет. Для действий по итогам
> доставки потребуется отдельный проход по журналу с подключением к ящику
> (например, в `mailspoon:pull`). Конфиг — опубликованный `config/mailspoon.php`.
