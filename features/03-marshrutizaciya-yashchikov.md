...
> `config/mailspoon.php`, проброс имени ящика через `Context`, миграция с
> `target`/`account` и уникальным индексом `(mailbox, target, fingerprint)`.
> Фаза 2 остаётся отложенной.
