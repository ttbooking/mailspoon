...
> **фаза 1** — маршрутизация один-к-одному, **фаза 2** — fan-out на несколько
> приложений (отложена до реальной потребности, YAGNI; поглощает
> [#13](13-fan-out-endpointy.md)). Схема БД готовится под фазу 2 сразу, чтобы
> не делать вторую ломающую миграцию.
>
> ✅ **Фаза 1 реализована** (после 3.0.0): карта `routes` в опубликованном
> `config/mailspoon.php`, миграция с `target`/`account` и уникальным индексом
> `(mailbox, target, fingerprint)`. Имя ящика несёт само событие
> `MessageReceived` — наш PR в imapengine-laravel принят и вышел в 1.3.0,
> поэтому описанный ниже обход через `Context` не понадобился. Фаза 2
> остаётся отложенной.
...
- [x] Письмо сохраняется с эндпоинтом, соответствующим его ящику, и
      доставляется на него.
- [x] `mailspoon:deliver` подписывает запрос ключом маршрута; ключи не хранятся в БД.
- [x] Откат на глобальный `endpoint/key`, если маршрут не задан (back-compat).
- [x] Колонка `mailbox` хранит имя ящика из `config/imap.php`; колонка `target`
      (= `default`) и уникальный индекс `(mailbox, target, fingerprint)`
      заложены миграцией (IMAP-логин — в новой колонке `account`).
- [x] Тест: два ящика → два разных `Http::fake()`-адреса с разными подписями.
- [x] Тест: одно письмо в двух ящиках доставляется на оба эндпоинта.
