...
> Актуализация (2.0/3.0): эскиз писался до store-and-forward — payload теперь
> формируется при доставке (`mailspoon:deliver` читает архивный MIME), а не в
> listener'е. Реализовывать как `MailgunFullFormatter` поверх интерфейса
> форматтеров из [#14](14-shablonizaciya-payload.md) — делать связкой, иначе
> придётся переделывать. Настройка — в опубликованном `config/mailspoon.php`
> (#21), а не env-флаги.
