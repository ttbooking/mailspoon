...
способ задания расписаний, имена artisan-команд (`mailspoon:*`) → **3.0.0**.
В UPGRADE.md — раздел 2.x → 3.0 (маппинг env-переменных на ключи конфига и
старых имён команд на новые; перенос данных не требуется — схема БД не
меняется).
