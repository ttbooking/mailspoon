...
> Актуализация (2.0/3.0): форматтер применяется при доставке
> (`mailspoon:deliver`), вход — архивный MIME и запись журнала, а не живое
> сообщение в listener'е. Реализовывать связкой с
> [#02](02-polnyy-payload-mailgun.md) — full-payload это первый нетривиальный
> форматтер. Свой форматтер хост указывает классом прямо в опубликованном
> `config/mailspoon.php` — пакетная модель упрощает расширение.
