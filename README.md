...
«Фильтрация писем») и `after` (действия по исходам, см. «Действия после
пересылки»; каждый исход откатывается на глобальный отдельно).
...
### Действия после пересылки (`after`)

Гигиена ящика-робота: что сделать с письмом в ящике, когда его исход известен —
глобально или на маршруте:

```php
'after' => [
    'delivered' => 'move:Processed', // успешно доставлено
    'failed'    => 'move:Failed',    // попытки доставки исчерпаны
    'filtered'  => 'delete',         // отсеяно фильтром
],
```

Действия: `none` (дефолт — ничего, текущее поведение), `seen`,
`keyword:<имя>`, `move:<папка>`, `delete`. Папка для `move:` создаётся
автоматически; `delete` лишь ставит флаг `\Deleted` — окончательно письмо
исчезнет при ближайшем expunge на сервере.

Исходы наступают в разные фазы store-and-forward, поэтому выполняются
по-разному:

- **`filtered`** известен прямо при захвате — действие выполняется сразу в
  listener (маркер ящика ставится в любом случае, до действия);
- **`delivered`** и **`failed`** наступают позже в `mailspoon:deliver`, без
  IMAP-соединения. Их применяет отдельная команда `mailspoon:tidy` (в
  расписании по умолчанию, `MAILSPOON_TIDY_CRON`): она находит письмо по UID,
  сохранённому при захвате, и сверяет его identity (`Message-ID` или
  fingerprint) — если письмо переехало, удалено вручную или сменился
  `UIDVALIDITY`, запись помечается пропущенной, чужое письмо не трогается.

`failed` означает **финальный** провал (попытки исчерпаны,
`MAILSPOON_MAX_ATTEMPTS`) — пока идут ретраи, письмо остаётся на месте.
`mailspoon:replay` сбрасывает состояние действия вместе со счётчиком попыток:
новый исход получит своё действие. Без настроенных действий `mailspoon:tidy` —
дешёвый no-op, к IMAP не подключается.

```dotenv
MAILSPOON_AFTER_DELIVERED=move:Processed
MAILSPOON_AFTER_FAILED=move:Failed
MAILSPOON_AFTER_FILTERED=none
MAILSPOON_TIDY_CRON="* * * * *"
```
...
### `mailspoon:tidy` — действия после пересылки

Применяет настроенные действия `after.delivered`/`after.failed` (см.
«Действия после пересылки») к письмам, чей исход стал финальным: подключается
к ящикам, находит каждое письмо по сохранённому UID, сверяет identity и
выполняет действие. Стоит в расписании по умолчанию; без настроенных действий
завершается мгновенно, не трогая IMAP.

```bash
php artisan mailspoon:tidy             # обработать накопившиеся исходы
php artisan mailspoon:tidy --limit=10  # ограничить пачку
```
...
ключ каждого ящика (маршрут или глобальные), маркер, фильтры и действия
`after`, реальный IMAP-логин и доступность эндпоинта. Печатает образец подписи для сверки ключа с получателем
